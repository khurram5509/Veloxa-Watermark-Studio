...
contextBridge.exposeInMainWorld('veloxa', {
  ...
  engine: {
    ...
    clearDone: () => invoke('engine:clearDone'),
    removeJob: (jobId) => invoke('engine:removeJob', jobId),
    removeJobs: (jobIds) => invoke('engine:removeJobs', jobIds),
    duplicateJobs: (jobIds) => invoke('engine:duplicateJobs', jobIds),
    retryRows: (jobIds) => invoke('engine:retryRows', jobIds),
